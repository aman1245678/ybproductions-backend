...
@Component({
  ...
  template: `
    <header
      class="fixed top-0 left-0 w-full z-50 transition-all duration-500 nav-shell"
      [class.nav-scrolled]="scrollService.isScrolled()"
      [class.nav-hidden]="isNavHidden()"
    >
      <!-- Main header row: logo + nav + login -->
      <div class="mx-auto flex max-w-[100rem] items-center gap-3 px-4 sm:px-6 lg:px-8 min-h-[3.75rem] lg:min-h-[4.25rem]">
        ...
        <nav class="hidden lg:flex flex-1 items-center justify-center gap-x-2.5 xl:gap-x-4 min-w-0 px-1" aria-label="Main">
          @for (link of primaryNavLinks; track link.path) {
            @if (link.path === '/about') {
              ...
              <a
                ...
                class="nav-link relative group shrink-0"
              >
                {{ link.label }}
          ...
          @for (link of moreNavLinks; track link.path) {
            <a [routerLink]="link.path" routerLinkActive="text-brand-gold" class="nav-link relative group shrink-0">
              {{ link.label }}
            </a>
          }
        </nav>

        <!-- Desktop auth — login only in header row -->
        <div class="hidden lg:flex shrink-0 items-center self-center pt-1">
          @if (auth.isLoggedIn()) {
            <div class="flex items-center gap-2 xl:gap-3">
              ...
              <span class="hidden xl:inline text-brand-white/70 font-poppins text-xs max-w-[6rem] truncate">{{ auth.user()?.fullName }}</span>
              <button type="button" (click)="logout()" class="px-4 py-2 border border-brand-gold/40 text-brand-gold font-poppins font-medium text-xs rounded-full hover:bg-brand-gold hover:text-brand-black transition-all whitespace-nowrap">Log Out</button>
            </div>
          } @else {
            <button
              type="button"
              (click)="openAuthModal()"
              class="px-4 py-2 bg-brand-gold text-brand-black font-poppins font-medium text-xs rounded-full hover:bg-brand-white transition-all whitespace-nowrap"
            >
              Log In / Sign Up
            </button>
          }
        ...
        <!-- Mobile menu toggle -->
        <button
          class="lg:hidden relative z-50 ml-auto w-9 h-9 flex items-center justify-center shrink-0"
          (click)="toggleMenu()"
      ...
      <!-- Get Started — below header bar (desktop) -->
      <div class="hidden lg:flex justify-end px-4 sm:px-6 lg:px-8 pb-3 pt-0.5 border-t border-brand-white/5">
        <a
          routerLink="/get-started"
          class="px-5 py-2 bg-brand-pink text-white font-poppins font-medium text-xs rounded-full hover:bg-brand-gold hover:text-brand-black transition-all whitespace-nowrap"
        >
          Get Started
        </a>
      </div>
      ...
      >
        <div class="w-full max-w-lg mx-auto px-5 pt-24 pb-8 md:px-8">
          <nav class="grid grid-cols-1 md:grid-cols-2 gap-1 md:gap-2" aria-label="Mobile">
            @for (link of navLinks; track link.path) {
              @if (link.path === '/about') {
                <div class="col-span-1 md:col-span-2 flex flex-col gap-2 py-1">
                  <button type="button" class="text-left text-lg font-playfair text-brand-white hover:text-brand-gold py-2" (click)="toggleMobileAbout()">About</button>
                  @if (mobileAboutOpen()) {
          ...
          <div class="mt-6 pt-5 border-t border-brand-white/10 flex flex-col items-stretch gap-3">
            @if (auth.isLoggedIn()) {
              ...
              <button
                type="button"
                class="w-full px-6 py-2.5 border border-brand-gold text-brand-gold font-poppins font-medium text-sm rounded-full hover:bg-brand-gold hover:text-brand-black transition-all"
                (click)="logout()"
              >
                Log Out
              </button>
              ...
              <button
                type="button"
                class="w-full px-6 py-2.5 bg-brand-gold text-brand-black font-poppins font-medium text-sm rounded-full hover:bg-brand-white transition-all"
                (click)="openAuthModal()"
              >
                Log In / Sign Up
              </button>
            ...
            <a
              routerLink="/get-started"
              class="w-full px-6 py-2.5 bg-brand-pink text-white font-poppins font-medium text-sm rounded-full hover:bg-brand-gold hover:text-brand-black transition-all text-center"
              (click)="closeMenu()"
            >
              Get Started
            </a>
            ...
            <div class="flex justify-center gap-3 mt-4">
              <a href="https://www.instagram.com/ybproductions2025?igsi=dXpqaWV3cmJleGxn" target="_blank" rel="noopener noreferrer" aria-label="Instagram" class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#f58529] via-[#dd2a7b] to-[#8134af] text-white shadow-lg transition-transform hover:scale-110">
              ...
              <a href="https://youtube.com/@yashvibaggaproductions?si=eKjUdxkHtL-AO52B" target="_blank" rel="noopener noreferrer" aria-label="YouTube" class="flex h-10 w-10 items-center justify-center rounded-xl bg-[#FF0000] text-white shadow-lg transition-transform hover:scale-110">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
              </a>
              <a href="https://www.facebook.com/yashvibaggaproductions" target="_blank" rel="noopener noreferrer" aria-label="Facebook" class="flex h-10 w-10 items-center justify-center rounded-xl bg-[#1877F2] text-white shadow-lg transition-transform hover:scale-110">
              ...
              <a href="https://www.linkedin.com/company/yashvi-bagga-productions" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" class="flex h-10 w-10 items-center justify-center rounded-xl bg-[#0A66C2] text-white shadow-lg transition-transform hover:scale-110">
